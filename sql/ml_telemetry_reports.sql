...
create or replace view public.ml_telemetry_flag_counts_30d as
select
  e.domain,
  e.event_name,
  e.pipeline,
...
create or replace view public.ml_telemetry_routing_misses_30d as
select
...
create or replace view public.ml_telemetry_reclassify_rate_30d as
with sessions as (
  select
    e.session_id,
    coalesce(e.region_id, 'unknown') as region_id,
...
create or replace view public.ml_telemetry_reclassify_mismatches_30d as
select
  e.id,
  e.created_at,
  e.region_id,
  e.payload->>'routing_label' as routing_label,
  e.payload->'prior_tflite_meta'->'genusTop'->0->>'genus' as tflite_top_genus,
  e.payload->'top_predictions'->0->>'label' as cloud_top_latin,
  e.flags
from public.ml_telemetry_events e
where e.created_at >= now() - interval '30 days'
  and e.event_name = 'cloud_reclassify'
  and 'reclassify_mismatch' = any (e.flags)
