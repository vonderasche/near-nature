/**
 * Upload the global v16 capture model to Supabase Storage (`region-models/global/`).
 *
 * Usage:
 *   npm run sync:v16-models
 *   npm run upload:global-v16
 */
...
const VERSION = process.argv[2] ?? '2026.07.1';
const ASSET_ROOT = join(root, 'assets/tflite/v16/tflite');

const BUNDLE_FILES = ['v16.tflite', 'labels.json', 'model_info.json'];
...
async function buildManifest() {
  ...
  for (const name of BUNDLE_FILES) {
    ...
    if (!existsSync(absPath)) {
      throw new Error(`Missing ${absPath}. Run npm run sync:v16-models first.`);
    }
    const relPath = `v16/tflite/${name}`;
...
async function main() {
  ...
  console.log(`Uploading v16 capture bundle (${Math.round(manifest.totalSizeBytes / 1_000_000)} MB)…`);
