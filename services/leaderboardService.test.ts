...
import {
  ...
  isPaginatedLeaderboardRpcMissing,
...
describe('isPaginatedLeaderboardRpcMissing', () => {
...
describe('fetchDetectionLeaderboardPage', () => {
  it('requests pageSize + 1 and trims the extra row when hasMore', async () => {
    ...
    const { rows, hasMore } = await fetchDetectionLeaderboardPage({
      offset: 0,
      pageSize: 1,
    });
  ...
  it('falls back to legacy no-arg RPC and slices client-side', async () => {
    ...
    const page0 = await fetchDetectionLeaderboardPage({ offset: 0, pageSize: 1 });
    expect(page0.rows).toHaveLength(1);
    expect(page0.rows[0]?.username).toBe('first');
    expect(page0.hasMore).toBe(true);

    const page1 = await fetchDetectionLeaderboardPage({ offset: 1, pageSize: 1 });
