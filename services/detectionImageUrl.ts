...
import {
  ...
  invalidatePersistedSignedUrl,
  loadPersistedSignedUrlMap,
...
export { clearSignedDetectionUrlCache, invalidateSignedDetectionUrlCacheForPath };

const SIGNED_URL_EXPIRY_SEC = 60 * 60 * 24; // 24h — aligned with in-memory cache TTL

/**
 * Clears cached signed URLs for a stored detections-bucket URL.
 * Call after overwriting an object at a fixed path (e.g. profile-avatar.jpg).
 */
export async function invalidateDetectionImageDisplayUrl(storedUrl: string): Promise<void> {
  const path = extractDetectionsObjectPathFromStoredUrl(storedUrl);
  if (!path) return;
  invalidateSignedDetectionUrlCacheForPath(path);
  await invalidatePersistedSignedUrl(path);
}
